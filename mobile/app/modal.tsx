...
export default function ModalScreen() {
  ...
  const onConfirmDate = (value: any) => {
    ...
    setDate(new Date(value.date.toISOString().split("T")[0]));
  };
  ...
  useEffect(() => {
    if (isEditing && params.expenseId && params.accountId && state) {
      ...
      if (monthData && monthData.accounts[acctId]) {
        const exp = monthData.accounts[acctId].expenses.find(
          (e) => e.id === expId,
        );
        if (exp) {
          setTitle(exp.title);
  ...
  const validate = () => {
    ...
    if (!dateRegex.test(date.toISOString().split("T")[0])) {
    ...
    <View style={styles.container}>
      ...
      >
          ...
          <View style={styles.amountRow}>
            <Text
              style={[
                ...
                !!errors.amount && { color: COLORS.danger },
              ]}
            >
              {currencySymbol}
            </Text>
        ...
        <View style={styles.section}>
          ...
          {/* Date */}
          <View style={styles.fieldGroup}>
            <Text style={styles.fieldLabel}>Transaction Date: {date ? date.toISOString().split("T")[0] : "N/A"}</Text>
            <SafeAreaView style={styles.datePickerContainer}>
                <Button onPress={() => setOpenDatePicker(true)} mode="contained">
                    Pick Date
                </Button>
                <DatePickerModal
                    locale="en"
              ...
              return (
                ...
                >
                  ...
                  <Text
                    style={[
                      styles.accountName,
                      isSelected && styles.accountNameActive,
                    ]}
                  >
                    {acct.name}
                  </Text>
                  <Text
                    style={[
                      styles.accountType,
                      isSelected && styles.accountTypeActive,
                    ]}
                  >
                    {acct.type}
                  </Text>
        ...
        <View style={styles.sectionHeader}>
          ...
          <Text style={styles.sectionSubtitle}>
            {targetAccount?.categories.length} available
          </Text>
        </View>
        <View style={styles.catGrid}>
          {targetAccount?.categories.map((c) => {
            ...
            return (
              ...
              >
                <View
                  style={[styles.catIconBg, isSel && styles.catIconBgActive]}
                >
                  <Ionicons
                ...
                <Text
                  style={[styles.catLabel, isSel && styles.catLabelActive]}
                  numberOfLines={1}
                >
                  {c}
                </Text>
